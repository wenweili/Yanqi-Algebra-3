...
\begin{example}
	The rings $\Z_p$ of $p$-adic integers ($p$: prime number) together with the usual $p$-adic valuations are standard examples of DVRs. The algebra of formal power series $\Bbbk\llbracket X\rrbracket$ is also a DVR: the valuation of $\sum_n a_n X^n$ is the smallest $n$ such that $a_n \neq 0$.
...
\begin{proposition}\label{prop:reg-local-dvr}
	...
	Conversely, every discrete valuation ring is regular local of dimension $1$.
\end{proposition}
\begin{proof}
...
\section{Auxiliary results on the total ring of fractions}
Let $R$ be a ring, henceforth assumed Noetherian. If there exists a non zero-divisor $t \in R$ and $\mathfrak{p} \in \text{Ass}(R/(t))$, we say $\mathfrak{p}$ is \emph{associated to a non zero-divisor}.

\begin{lemma}\label{prop:zero-test-ass}
	Let $M$ be a finitely generated $R$-module. An element $x \in M$ is zero if and only if its image in $M_{\mathfrak{p}}$ is zero for every maximal element $\mathfrak{p}$ in $\text{Ass}(M)$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
	Suppose $R$ is reduced, then $\text{Ass}(R)$ consists of minimal primes.
\end{lemma}
\begin{proof}
...
\begin{proposition}\label{prop:normality-principal}
	Let $R$ be a Noetherian domain. Then $R$ is normal if and only if for every principal ideal $(t) \subset R$ and every $\mathfrak{p} \in \text{Ass}(R/(t))$, the ideal $\mathfrak{p}R_{\mathfrak{p}}$ is principal.
\end{proposition}
\begin{proof}
...
\begin{proof}
	...
	(i) $\implies$ (ii). Under the normality assumption, choose any non-invertible $t \in R \smallsetminus \{0\}$. Since $\dim R = 1$ and $\{0\}$ is a prime ideal, the associated prime of $(t)$ can only be the maximal ideal $\mathfrak{m}$, which is principal by Proposition \ref{prop:normality-principal}. This shows that $R$ is regular local.
...
\begin{lemma}\label{prop:reduced-test}
	...
	\begin{compactitem}
		\item the primes in $\text{Ass}(R)$ are all minimal, and
...
\begin{theorem}[J.-P. Serre]\label{prop:Serre-criterion}\index{Serre's criterion}
	...
	\begin{description}
		...
		\item[S2] For every non zero-divisor $t$ of $R$, the primes in $\text{Ass}(R/(t))$ are all of height $1$; the primes in $\text{Ass}(R)$ are all of height $0$.
	\end{description}
\end{theorem}
...
\begin{proof}
	...
	If $\text{depth}_{\mathfrak{p}}(R)=0$, there exist $\mathfrak{p}' \supset \mathfrak{p}$ such that $\mathfrak{p}' \in \text{Ass}(R)$ by Proposition \ref{prop:depth-zero}, but \textbf{S2} implies $\text{ht}(\mathfrak{p}) \leq \text{ht}(\mathfrak{p}') = 0$ which is absurd. If $\text{depth}_{\mathfrak{p}}(R)=1$, there exists a non zero-divisor $x$ in $R$ such that $\text{depth}_{\mathfrak{p}}(R/(x)) = 0$. The same argument furnishes $\mathfrak{p}' \supset \mathfrak{p}$ such that $\mathfrak{p}' \in \text{Ass}(R/(x))$. Now \textbf{S2} implies $\text{ht}(\mathfrak{p}) \leq \text{ht}(\mathfrak{p}')=1$, again a contradiction.
\end{proof}

...
\[ F^i \hat{M} := \Ker\left[ p_i: \hat{M} \to M/F^i M \right] = \left\{ (x_n)_n \in \hat{M}: n \leq i \implies x_i=0 \right\}, \]
...
Observe that any homomorphism $\varphi: M \to N$ between $R$-modules is automatically $\mathfrak{a}$-adically continuous, as $\varphi(\mathfrak{a}^n M) \subset \mathfrak{a}^n N$.
...
\begin{proof}
	...
	\[\begin{tikzcd}
R^{\oplus a} \dotimes{R} \hat{R} \arrow[r] \arrow[d] & R^{\oplus b} \dotimes{R} \hat{R} \arrow[r] \arrow[d] & M \dotimes{R} \hat{R} \arrow[r] \arrow[d] & 0 \\
...
\begin{proposition}\label{prop:completion-rad}
	The ring $\hat{R}$ is also Noetherian, and $\hat{I} \subset \text{rad}(\hat{R})$.
\end{proposition}
\begin{proof}
	Let $\mathfrak{A}$ be any ideal of $\hat{R}$, equipped with the filtration $F^n \mathfrak{A} := \hat{I}^n \cap \mathfrak{A}$. We have to show $\mathfrak{A}$ is finitely generated. Since $\gr_I(R) = \gr_{\hat{I}}(\hat{R})$ is Noetherian, so is $\gr_F(\mathfrak{A})$. Take $t_1, \ldots, t_n \in \mathfrak{A}$, $t_i \in F^{d_i} \mathfrak{A}$, whose images $\bar{t}_i$ in $\gr^{d_i}_F(\mathfrak{A})$ generate $\gr_F(\mathfrak{A})$. Using an appropriately shifted filtration on $L := \hat{R}^{\oplus n}$, we obtain a filtered homomorphism $\varphi: L \to  \mathfrak{A}$ with image $(t_1, \ldots, t_n)$, such that $\gr(\varphi)$ is surjective. Now apply the previous Lemma to obtain the first assertion.
	
	One of the characterizations of Jacobson radical says that $\hat{I} \subset \text{rad}(\hat{R})$ if and only if $1 - \hat{I} \subset \hat{R}^\times$. This is verified by noting that $(1-t)^{-1} = 1+t+t^2+ \cdots$ converges $I$-adically. This proves the second assertion.
\end{proof}
...
\begin{itemize}
	\item everything is graded by $\Gamma = (\Z_{\geq 0}^N, +)$ for some fixed $N$,
	\item $R_0/R_0 \cap \text{ann}(M)$ is an Artinian ring,
	\item $R$ is finitely generated over $R_0$.
\end{itemize}
The appearance of $\text{ann}(M)$ is harmless since $R$ can be safely replaced by $R/\text{ann}(M)$, which is legitimate since $\text{ann}(M)$ is a graded ideal of $R$. To see this, write $\text{ann}(M)$ as the intersection of $\text{ann}(x)$ where $x$ ranges over the homogeneous elements of $M$, and observe that $\text{ann}(x)$ must be graded.
...
\end{definition}
Parameter ideals are often called \emph{ideals of definition}. Here we follow the terminology of \cite{Eis95}.
...
\begin{lemma}\label{prop:para-ideal-characterization}
	An ideal $I \subset \text{rad}(R)$ is a parameter ideal for $M$ if and only if there exists $k$ with $\text{rad}(R)^k \subset I + \text{ann}(M)$. In this case $R/(I+\text{ann}(M))$ is an Artinian ring.
	
	In particular, $\text{rad}(R)$ is a parameter ideal for any $M$.
\end{lemma}
\begin{proof}
...
\end{compactitem}
Upon recalling Lemma \ref{prop:Hilbert-poly-0}, we are justified in defining
...
\begin{lemma}\label{prop:Hilbert-poly-I}
	...
	\begin{enumerate}[(i)]
		...
		\item In a short exact sequence $0 \to M' \to M \to M'' \to 0$ of finitely generated $R$-modules, we have
		\[ \deg H_I(M', \cdot), \deg H_I(M'', \cdot) \leq \deg H_I(M, \cdot), \]
		and $H_I(M, \cdot) - H_I(M', \cdot) - H_I(M'', \cdot)$ has degree $< d(M)$.
	\end{enumerate}
\end{lemma}
\begin{proof}
	(i): To compare the graded objects associated to two parameter ideals $I, J$, we apply the characterization in Lemma \ref{prop:para-ideal-characterization}: it suffices to take $J = \text{rad}(R)$, so that
...
\begin{proof}
	...
	\[ \frac{\mathfrak{p}_1}{Rt + \mathfrak{p}} \subsetneq \cdots \subsetneq \frac{\mathfrak{p}_m}{Rt + \mathfrak{p}} \]
	...
	Next, let us show $s(M) \leq \dim M$. Set $r := \dim M$. We contend that there exist $t_1, \ldots, t_r \in \text{rad}(R)$ such that $M/(t_1, \ldots, t_r)M$ has finite length; therefore $s \leq r$. When $r=0$ this follows from Lemma \ref{prop:dim-M-equiv}. For $r > 0$, we have $\text{rad}(R) \not\subset \mathfrak{p}$ for any minimal $\mathfrak{p} \in \text{Ass}(M)$ verifying $\dim R/\mathfrak{p} = \dim M$, for otherwise $\mathfrak{p}$ will contain, thus equal to a maximal ideal as $R$ is semi-local, and we would get $\dim M = 0$. Using prime avoidance (Proposition \ref{prop:prime-avoidance} applied to $I := \text{rad}(R)$ and the primes $\mathfrak{p}$ above), we may pick $t_1 \in \text{rad}(R)$ that does not belong to any $\mathfrak{p}$ above. From $\text{ann}(M/t_1 M) \supset Rt_1 + \text{ann}(M)$ and our choice of $t_1$, we see $\dim M/t_1 M < \dim M$. Our claim results from induction on $r$.
...
\begin{theorem}[Krull]
	Suppose $\mathfrak{a} = (t_1, \ldots, t_r)$ is a proper ideal of $R$, then for every minimal over-prime ideal $\mathfrak{p}$ of $\mathfrak{a}$, we have $\text{ht}(\mathfrak{p}) \leq r$.
\end{theorem}
...
\begin{definition}\index{regular local ring}
	We say a Noetherian local ring $R$ is a \emph{regular local ring} if $\mathfrak{m}$ can be generated by $d = \dim R$ elements $t_1, \ldots, t_d$. In this case we say $t_1, \ldots, t_d$ form a \emph{regular system of parameters}.
\end{definition}
